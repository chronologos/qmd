...
// Re-export types and utilities from llm-types for backward compatibility
// This allows existing imports from "./llm.js" to continue working
export {
  type TokenLogProb,
 ...
 * Note: Session management currently only works with local LlamaCpp, not remote LLM.
 */
function getSessionManager(): LLMSessionManager {
  const llm = getDefaultLlamaCpp();
  // Session management requires LlamaCpp; for remote LLM this may need adaptation
  if (!(llm instanceof LlamaCpp)) {
    throw new Error("Session management currently only supports local LlamaCpp");
  }
