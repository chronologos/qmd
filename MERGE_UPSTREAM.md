...
Fork-specific code lives in **fork-only files** that upstream doesn't have. This means upstream merges should produce **zero conflicts** (or at most a trivial one in `src/cli/qmd.ts`).
...
If there are conflicts, they should only be in `src/cli/qmd.ts` (the one file we modify). Resolve by keeping both: upstream's changes plus our fork additions.

## Step 4: Resolve conflicts (if any)

The only likely conflict is in `src/cli/qmd.ts`. Look for our fork additions:
...
| Minimal `src/cli/qmd.ts` modification (LLM + Anki hooks) | No remote/Anki flags |
